...
struct ClassroomHubView: View {
    ...
    var body: some View {
        ...
        .sheet(item: $sheet) { item in switch item { case .newClass: MiniProgramClassCreateView(courses: courses) { await load() }; case .newStudent: MiniProgramStudentCreateView { await load() }; case .newCourse: NavigationStack { CourseEditorView { await load() } }; case .join: ParentCourseAddView(students: students) { await load() }; case .bind: BindStudentSheet { await load() } } }
