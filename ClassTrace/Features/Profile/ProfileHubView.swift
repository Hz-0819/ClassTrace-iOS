...
struct ProfileHubView: View {
    ...
    var body: some View {
        List {
            Section {
                HStack(spacing: 14) { Image(systemName: "person.crop.circle.fill").font(.system(size: 52)).foregroundStyle(Color.ctBrand); VStack(alignment: .leading) { Text(session.user?.displayName ?? "用户").font(.title3.bold()); Text((session.user?.roles ?? []).map { $0.role.localizedStatus }.joined(separator: " · ")).foregroundStyle(Color.ctTextSecondary) } }
            }
            Section("服务") {
                NavigationLink { NotificationCenterView() } label: { Label("消息通知", systemImage: "bell") }
                NavigationLink { CommerceCenterView() } label: { Label("账单与退款", systemImage: "creditcard") }
                NavigationLink { VIPCenterView() } label: { Label("VIP 权益", systemImage: "crown") }
                NavigationLink { BusinessOverviewView() } label: { Label("经营概览", systemImage: "chart.bar") }
                NavigationLink { AccountSettingsView() } label: { Label("账号设置", systemImage: "person.crop.circle.badge.gearshape") }
                NavigationLink { ManualScheduleView() } label: { Label("个人日程", systemImage: "calendar") }
                NavigationLink { AnnouncementCenterView() } label: { Label("公告", systemImage: "megaphone") }
            }
            Section("支持") {
                NavigationLink { FeedbackCenterView() } label: { Label("意见反馈", systemImage: "bubble.left.and.text.bubble.right") }
                NavigationLink { AboutView() } label: { Label("关于课迹", systemImage: "info.circle") }
            }
            if session.user?.roles?.contains(where: { $0.role == "ADMIN" }) == true { Section("管理") { NavigationLink { AdminCenterView() } label: { Label("管理员工具", systemImage: "wrench.and.screwdriver") } } }
            Section { Button("退出登录", role: .destructive) { Task { await session.signOut(using: AuthRepository(client: dependencies.client, vault: dependencies.sessionVault)) } } }
        }.navigationTitle("我的")
    }
}
...
private struct VIPCenterView: View {
...
private struct BusinessOverviewView: View {
...
private struct FeedbackCenterView: View {
